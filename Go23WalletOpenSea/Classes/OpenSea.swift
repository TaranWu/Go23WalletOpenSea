...
public class OpenSea {
    ...
    public func fetchAssetsPromise(address owner: DerbyWallet.Address, chainId: ChainId, excludeContracts: [(DerbyWallet.Address, ChainId)]) -> Promise<Response<OpenSeaAddressesToNonFungibles>> {
        ...
        return when(resolved: [assetsPromise.asVoid(), collectionsPromise.asVoid()])
                .map(on: queue, { _ -> Response<OpenSeaAddressesToNonFungibles> in
                    ...
                    var result: [DerbyWallet.Address: [OpenSeaNonFungible]] = [:]
    ...
    private func fetchCollectionsPage(forOwner owner: DerbyWallet.Address, chainId: ChainId, offset: Int, sum: [CollectionKey: Collection] = [:]) -> Promise<Response<[CollectionKey: Collection]>> {
        let baseURL = getBaseURLForOpenSea(forChainId: chainId)
    ...
    private func fetchAssetsPage(forOwner owner: DerbyWallet.Address, chainId: ChainId, offset: Int, assets: OpenSeaAddressesToNonFungibles = [:], excludeContracts: [(DerbyWallet.Address, ChainId)]) -> Promise<Response<OpenSeaAddressesToNonFungibles>> {
        let baseURL = getBaseURLForOpenSea(forChainId: chainId)
...
//TODO extract to DerbyWalletCore or somewhere else
