...
struct NftAssetsFilter {
    ...
    func assets(excluding excludeContracts: [(Go23Wallet.Address, ChainId)]) -> [Go23Wallet.Address: [NftAsset]] {
...
extension NftAsset {
    init?(json: JSON) {
        ...
        guard let contract = Go23Wallet.Address(string: assetContractJson["address"].stringValue) else {
            return nil
        }
