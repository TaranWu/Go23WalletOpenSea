...
public struct OpenSeaAssetDecoder {
    ...
    static func decode(json: JSON, assets: [DerbyWallet.Address: [OpenSeaNonFungible]]) -> [DerbyWallet.Address: [OpenSeaNonFungible]] {
        var assets = assets
        ...
        for (_, each): (String, JSON) in json["assets"] {
            ...
            guard let contract = DerbyWallet.Address(string: assetContractJson["address"].stringValue) else {
                continue
            }
